...
class DataCollector:
    ...
    def get_front_month_future(self, symbol: str = "NQ") -> Future:
        """
        Get the front month future contract.
        
        Args:
            symbol: Future symbol (default NQ)
            
        Returns:
            Front month Future contract
            
        Raises:
            Exception: If contract cannot be qualified
        """
        # Create a continuous future to get the front month
        future = Future(symbol, exchange="CME", includeExpired=False)
        
        # Request contract details to get all available contracts
        contracts = self.ib.reqContractDetails(future)
        
        if not contracts:
            raise Exception(f"Could not find any {symbol} futures contracts")
        ...
        self.logger.info(f"Using front month contract: {front_month.localSymbol} (expires {front_month.lastTradeDateOrContractMonth})")
    ...
    def collect_historical_data(
        ...
        try:
            ...
            contract = self.get_front_month_future(symbol)
    ...
    def collect_option_chain(
        ...
        try:
            ...
            future = self.get_front_month_future(symbol)
    ...
    def collect_vix_data(self) -> Dict[str, float]:
        """
    ...
    def collect_account_snapshot(self) -> Dict[str, float]:
        """
    ...
    def collect_market_snapshot(self) -> MarketSnapshot:
        """
        ...
        future = self.get_front_month_future("NQ")
        ...
        option_chain = self.collect_option_chain()
        ...
        account_data = self.collect_account_snapshot()
    ...
    def run_collection_session(
        ...
        try:
            self.collect_historical_data(days=60)
            self.collect_intraday_data(interval=5)
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Market Data Collector for 0DTE Trading System')
    parser.add_argument('--days', type=int, default=60, help='Number of days of historical data to collect')
    parser.add_argument('--interval', type=int, default=5, help='Minutes between snapshots')
    parser.add_argument('--duration', type=float, default=6.5, help='Hours to run collection')
    parser.add_argument('--paper', action='store_true', default=True, help='Use paper trading connection')
    parser.add_argument('--live', dest='paper', action='store_false', help='Use live trading connection')
    parser.add_argument('--data-dir', type=str, default='market_data', help='Directory to save data')
    ...
    try:
        # Connect to IB
        connected = collector.connect(paper=args.paper)
